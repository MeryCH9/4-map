...
public class Bigrams {
	...
	public Bigrams(String fn) {
        ...
        try {
            ...
            while (sc.hasNext()) {
                ...
                if (prevWord != null) {
                    Pair<String, String> p = new Pair<>(prevWord, word);
    ...
    public float lookupBigram(String w1, String w2) {
		...
		Pair<String, String> p = new Pair<>(w1, w2);
        if (!bigramCounts.containsKey(p)) {
			System.out.println("Bigram not found.");
			System.out.printf(w1 + " appears %.2f times. \n" , unigramCounts.get(w1));
            if(bigramCounts.containsKey(p)){
                System.out.printf("(" + w1 + ", " + w2 + ") appears %.2f times\n" , bigramCounts.get(p));
            }else{
                System.out.printf("(" + w1 + ", " + w2 + ") appears %.2f times\n", 0.0f);
	...
	System.out.printf(w1+ " appears %.2f times. \n",unigramCounts.get(w1));
	System.out.printf("(" + w1 + ", " + w2 + ") appears %.2f times. \n",bigramCounts.get(p));
	System.out.printf("%.4f \n", (float) bigramCounts.get(p) / unigramCounts.get(w1));
    return (float) 0.0;
}
